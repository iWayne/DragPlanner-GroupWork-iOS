...
//        (self.view as MADayView).scrollView.addGestureRecognizer(tapScrollView)
    ...
    //---------------------------------------------------------------------------------------------------
    func swipeRight() {
        arrEvent.removeAll(keepCapacity: true) // new line
        var dayView:MADayView = self.view as MADayView
    ...
    @IBAction func doneButtonAction(sender: AnyObject) {
        ...
        var theEvent = self.addEventByDrop(topY, bottomY1: bottomY)
        moveTextView.hidden = true
        ...
        if !modifying {
            ...
            testObject["startTime"] = changeCGFloatToTime(topY).dateByAddingTimeInterval(4*60*60*(-1))
            testObject["endTime"] = changeCGFloatToTime(bottomY).dateByAddingTimeInterval(4*60*60*(-1))
//            testObject["eventId"] = testObject.objectId
...
//                        for object in objects {
//                            if count == 0 {
//                                let objID = object.objectId as String
////                                self.idMayDelete = objID
////                                println("sasasa:\(self.idMayDelete)")
//                                var query = PFQuery(className:"event")
...
//        println("dsfdfdsf")
    ...
    func addEventByDrop(topY1:CGFloat,bottomY1:CGFloat) ->MAEvent{
        var dayViewOffsetY = (self.view as MADayView).scrollView.contentOffset.y
        ...
        var theEvent = self.createEvent(startTime!, endTime: endTime!, color: moveTextView.backgroundColor!)
...
//        var query = PFQuery(className:"event")
//        query.whereKey("eventId", equalTo:self.tempEvent?.start.dateByAddingTimeInterval(60*60*(-4)))
//        query.findObjectsInBackgroundWithBlock {
//            (objects: [AnyObject]?, error: NSError?) -> Void in
//            
//            if error == nil {
