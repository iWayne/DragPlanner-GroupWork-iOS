    ...
    @IBAction func doneButtonAction(sender: AnyObject) {
        ...
        if !modifying {
            ...
            query.findObjectsInBackgroundWithBlock {
                ...
                if error == nil {
                    // The find succeeded.
                    println("Successfully retrieved \(objects.count) Pizzas.")
                    var count = 0
                    // Do something with the found objects
                    if let objects = objects as? [PFObject] {
                        for object in objects {
...
//            query2.getObjectInBackgroundWithId(objId) {
//                (object: PFObject?, error: NSError?) -> Void in
//                if error != nil {
//                    println(error)
//                } else if let object = object {
//                    println("I can be here")
//                    object["eventContent"] = self.moveTextView.text
//                    object["eventColor"] = self.moveTextView.backgroundColor
//                    object["startTime"] = self.tempEvent?.start.dateByAddingTimeInterval(60*60*(-4))
//                    object["endTime"] = self.tempEvent?.end.dateByAddingTimeInterval(60*60*(-4))
